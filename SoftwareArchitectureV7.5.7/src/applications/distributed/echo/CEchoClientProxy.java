...
public class CEchoClientProxy extends CClientProxy {
	private Queue invPToInvRQueue = new Queue();
	private Queue terRToTerPQueue = new Queue();
	private String host = "localhost"; // TODO
	private int port = Utils.ECHO_SERVICE_PORT; // TODO

	public CEchoClientProxy(String name) {
		super(name);
	}
	...
	public void i_PosInvP(Queue local) {
		...
		try {
			...
			case "echo":
				...
				break;
				default:
					new MyError("Operation '" + inRequest.getOp() + "' is not implemented by CRH", Utils.FATAL_ERROR).print();					break;
			}
